...
// Fullscreen Matrix Rain Animation Engine
(function initMatrixRain() {
  ...
  function draw() {
    // Semi-transparent bg to create trail effect
    const isLight = document.body.classList.contains('light-theme');
    ctx.fillStyle = isLight ? 'rgba(255, 255, 255, 0.05)' : 'rgba(3, 3, 3, 0.05)';
    ctx.fillRect(0, 0, canvas.width, canvas.height);

    // Text color matching the active theme
...
(function initTheme() {
  ...
  function setTheme(theme) {
    ...
    localStorage.setItem('template6-theme', theme);
  }

  // Load saved theme or default to dark
  const savedTheme = localStorage.getItem('template6-theme') || 'dark';
  setTheme(savedTheme);
...
(function hydratePortfolio() {
  ...
  if (nameEl && data.name) {
    ...
    document.title = `${data.name} | Terminal`;
    
    // Logo text
    const logoText = document.getElementById('logo-text');
    if (logoText) {
      logoText.textContent = `ROOT@${data.name.split(' ')[0].toUpperCase()}:~$`;
    }
